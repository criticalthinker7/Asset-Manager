...
const PRICING_TIERS = [
  {
    ...
    price: "$19.99",
    cadence: "/mo",
    description: "For creators who need a personal grant discovery workspace.",
    highlight: "Best for solo artists",
    featured: false,
    features: [
      "Curated Canadian grant discovery",
      "Saved grants, filters, and deadline tracking",
      "Basic AI grant assistant for eligibility and drafts",
      "512 GB secure document storage",
    ],
  },
  {
    tier: "Tier 2",
    ...
    price: "$99.99",
    cadence: "/mo",
    description: "For producers managing more files, collaborators, and active applications.",
    highlight: "Producer workspace",
    featured: false,
    features: [
      "Enhanced AI models and priority grant database updates",
      "Secure document vault for applications and support materials",
      "Producer pipeline tracking across projects",
      "1 TB storage for grant packages and creative files",
    ],
  },
  {
    tier: "Tier 3",
    ...
    price: "$199.99",
    cadence: "/mo",
    description: "For small companies coordinating multiple projects and funder relationships.",
    highlight: "Team-ready",
    featured: true,
    features: [
      "Advanced integrations and team workflows",
      "Multi-project pipeline and application management",
      "Higher storage and AI usage limits",
      "Company-level reporting for deadlines, statuses, and funder fit",
    ],
  },
  {
    tier: "Tier 4",
    name: "Hubs & consultants",
    price: "$499.99",
    cadence: "/mo +",
    description: "For consultants, incubators, and arts hubs supporting many creators.",
    highlight: "Multi-client scale",
    featured: false,
    features: [
      "Multi-client portfolio management",
      "Complex funder modelling and opportunity mapping",
      "Add-ons for consulting, onboarding, and custom workflows",
      "Premium storage for high-volume document libraries",
    ],
  },
] as const;
...
function PublicSiteHeader({ onSignIn, onGetStarted }: { onSignIn?: () => void; onGetStarted?: () => void }) {
  const activePath = getCurrentAppPath();
      ...
      <div className="site-header-inner" style={{ maxWidth:1760, margin:"0 auto", padding:"18px 32px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:24 }}>
        ...
        <nav className="site-header-nav" aria-label="Primary navigation" style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:22, flex:1 }}>
          {PUBLIC_NAV_ITEMS.map(item => {
            ...
            return (
            <a key={item.href} href={withBasePath(item.href)} style={{ color:isActive?THEME.chartreuse:"rgba(247,246,241,0.88)", textDecoration:"none", fontSize:22, fontWeight:700, lineHeight:1.15, whiteSpace:"nowrap", borderBottom:isActive?`2px solid ${THEME.chartreuse}`:"2px solid transparent", paddingBottom:4 }}>
              {item.label}
            </a>
          );})}
        </nav>
...
function PricingInfoPage() {
  return (
    <div style={{ minHeight:"100vh", background:THEME.navy, color:THEME.ivory, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <PublicSiteHeader />
      <main style={{ maxWidth:1280, margin:"0 auto", padding:"58px 24px 96px" }}>
        ...
        <div style={{ color:THEME.chartreuse, fontSize:12, fontWeight:800, letterSpacing:"3px", textTransform:"uppercase", marginBottom:16 }}>Pricing</div>
        <div style={{ display:"grid", gridTemplateColumns:"minmax(0, 1.05fr) minmax(280px, 0.95fr)", gap:38, alignItems:"end", marginBottom:42 }}>
          <div>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(46px,7vw,86px)", lineHeight:0.94, fontWeight:700, margin:"0 0 18px", maxWidth:760 }}>A subscription model that grows with creator complexity.</h1>
            <p style={{ color:"rgba(247,246,241,0.72)", fontSize:18, lineHeight:1.7, margin:0, maxWidth:720 }}>Start with individual grant discovery, then scale into producer workflows, company pipelines, and multi-client consultant management.</p>
          </div>
          <div style={{ background:"rgba(255,255,255,0.04)", border:`1px solid ${THEME.line}`, padding:"24px 26px", alignSelf:"stretch", display:"flex", flexDirection:"column", justifyContent:"center" }}>
            <div style={{ color:THEME.chartreuse, fontSize:11, fontWeight:800, letterSpacing:"2.5px", textTransform:"uppercase", marginBottom:12 }}>Launch note</div>
            <p style={{ margin:"0 0 14px", color:THEME.ivory, fontSize:15, lineHeight:1.75 }}>Simple monthly subscriptions keep CanGrants easy to adopt while usage patterns mature.</p>
            <p style={{ margin:0, color:"rgba(247,246,241,0.66)", fontSize:14, lineHeight:1.7 }}>Higher tiers are tied to storage, AI usage, collaboration, integrations, and multi-client workflows.</p>
          </div>
        </div>

        <section aria-label="CanGrants subscription tiers" style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(250px, 1fr))", gap:18, marginBottom:34 }}>
          {PRICING_TIERS.map((plan, index) => (
            <article
              ...
              style={{
                ...
                gap:18,
                padding:"28px",
                background:plan.featured ? "linear-gradient(145deg, rgba(238,228,78,0.2), rgba(255,255,255,0.055))" : "rgba(255,255,255,0.045)",
                border:`1px solid ${plan.featured ? "rgba(238,228,78,0.58)" : THEME.line}`,
                boxShadow:plan.featured ? "0 22px 60px rgba(0,0,0,0.26)" : "none",
                position:"relative",
              }}
            >
              {plan.featured && (
                <div style={{ position:"absolute", top:18, right:18, background:THEME.chartreuse, color:THEME.foreground, fontSize:11, fontWeight:900, letterSpacing:"1.4px", textTransform:"uppercase", padding:"7px 10px" }}>Popular</div>
              )}
              <div>
                <div style={{ color:index === 0 ? "#C8A84B" : THEME.chartreuse, fontSize:12, fontWeight:900, letterSpacing:"2.4px", textTransform:"uppercase", marginBottom:18 }}>{plan.tier}</div>
                <div style={{ display:"flex", alignItems:"baseline", gap:4, marginBottom:8 }}>
                  <span style={{ fontSize:"clamp(32px,4vw,42px)", fontWeight:900, color:THEME.ivory, letterSpacing:"-1.5px" }}>{plan.price}</span>
                  <span style={{ color:"rgba(247,246,241,0.65)", fontSize:18, fontWeight:700 }}>{plan.cadence}</span>
                </div>
                <h2 style={{ margin:"0 0 10px", color:THEME.ivory, fontSize:20, lineHeight:1.25, fontWeight:900 }}>{plan.name}</h2>
                <p style={{ margin:0, color:"rgba(247,246,241,0.62)", fontSize:14, lineHeight:1.65 }}>{plan.description}</p>
              </div>
              <div style={{ borderTop:`1px solid ${THEME.line}`, paddingTop:18, flex:1 }}>
                <div style={{ color:"#C8A84B", fontSize:12, fontWeight:900, marginBottom:12 }}>{plan.highlight}</div>
                <ul style={{ listStyle:"none", padding:0, margin:0, display:"grid", gap:11 }}>
                  {plan.features.map(feature => (
                    <li key={feature} style={{ display:"grid", gridTemplateColumns:"18px 1fr", gap:9, color:"rgba(247,246,241,0.78)", fontSize:14, lineHeight:1.5 }}>
                      <span aria-hidden="true" style={{ color:THEME.chartreuse, fontWeight:900 }}>✓</span>
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
              ...
              <a
                href={plan.name === "Hubs & consultants" ? withBasePath("/ask-a-consultant") : `${withBasePath("/")}?register=1`}
                style={{
                  marginTop:"auto",
                  display:"inline-flex",
                  justifyContent:"center",
                  textDecoration:"none",
                  background:plan.featured ? THEME.chartreuse : "transparent",
                  color:plan.featured ? THEME.foreground : THEME.chartreuse,
                  border:`1px solid ${plan.featured ? THEME.chartreuse : "rgba(238,228,78,0.45)"}`,
                  padding:"13px 16px",
                  fontSize:14,
                  fontWeight:900,
                }}
              >
                {plan.name === "Hubs & consultants" ? "Talk to us" : "Get started"}
              </a>
            </article>
          ))}
        </section>
        ...
        <section style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(260px, 1fr))", gap:18 }}>
          <div style={{ background:"rgba(255,255,255,0.035)", border:`1px solid ${THEME.line}`, padding:"24px 26px" }}>
            <div style={{ color:THEME.chartreuse, fontSize:11, fontWeight:900, letterSpacing:"2.4px", textTransform:"uppercase", marginBottom:10 }}>Included direction</div>
            <p style={{ color:"rgba(247,246,241,0.76)", fontSize:15, lineHeight:1.75, margin:0 }}>Each plan is shaped around the same core journey: discover relevant grants, save and filter opportunities, track deadlines, and use AI support to move from idea to application.</p>
          </div>
          <div style={{ background:"rgba(255,255,255,0.035)", border:`1px solid ${THEME.line}`, padding:"24px 26px" }}>
            <div style={{ color:THEME.chartreuse, fontSize:11, fontWeight:900, letterSpacing:"2.4px", textTransform:"uppercase", marginBottom:10 }}>Need a custom path?</div>
            <p style={{ color:"rgba(247,246,241,0.76)", fontSize:15, lineHeight:1.75, margin:"0 0 16px" }}>Consultants, arts hubs, and producing teams can pair the subscription with onboarding, grant strategy, and custom workflow support.</p>
            <a href={withBasePath("/contact")} style={{ color:THEME.chartreuse, fontWeight:900, textDecoration:"none", fontSize:14 }}>Contact CanGrants →</a>
          </div>
        </section>
      </main>
    </div>
...
function PublicInfoPage({ path }: { path: string }) {
  ...
  if (normalized === "/pricing") return <PricingInfoPage />;
